...
functions that don't return values. The latter are often called _procedures_. In
...
In Carp I'm doing the same thing except that you'll be able to omit the return
...
In Carp, we're using multiple passes so that global variables and functions can
...
variables. This is usually called a _stack frame_. Each time you return from a
function, the top most stack frame is popped off of that stack.

In Carp, we're doing the same thing:

1. When calling a function, a new set of locals is initialized. All parameters
   are added to that new frame and that frame is pushed.
...
change the binder to create different kind of symbols for
